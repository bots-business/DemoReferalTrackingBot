...
function saveActiveUsers(userKey, refUser){
  ...
  if(!activity){
    activityList[userKey] = { count:0, username: refUser.username, telegramid: refUser.telegramid, first_name: refUser.first_name }
  }
...
function getTopList(top_count=10){
  ...
  for(key in activityList){
    ...
    telegramid = activityList[key].telegramid;
    first_name = activityList[key].first_name;
    sortedList.push(
      { count: count, userKey: key, username:username, telegramid:telegramid, first_name:first_name }
    );
  }
...
function track(_trackOptions={}){
  let need_track = (message.split(' ')[0]=='/start')&¶ms;
  if(!need_track){
